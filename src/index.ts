...
// Directly create the instance right here to ensure the server starts perfectly
const finalBuilder = new addonBuilder(manifest);
finalBuilder.defineCatalogHandler(builder.defineCatalogHandler as any);
finalBuilder.defineMetaHandler(builder.defineMetaHandler as any);

serveHTTP(finalBuilder.getInterface(), { port });
console.log(`Anime Addon successfully running on cloud port ${port}`);
